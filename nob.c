...
int main(int argc, char **argv)
{
    ...
    // Compilation du test
    {
        // Check if the test is running and kill it before rebuilding
        Nob_Cmd kill_cmd = {0};
        nob_cmd_append(&kill_cmd, "taskkill", "/F", "/IM", "test.exe", "/T");
        ...
        #endif
        #if DEBUG_OPTION
        nob_cmd_append(&cmd, "-g", "-mwindows");
        #endif
        nob_cmd_append(&cmd, "./src/test.c");
